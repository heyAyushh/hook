...
- **Proactive watching** — A GitHub or Linear sub-agent can monitor the relay (incoming webhook flow), poll APIs, or react to events without the parent `agent` having to handle every source.
...
|------------|-------------------------|
| **GitHub** | Proactively watch PR/review events flowing through the relay; react (review, comment); optionally invoke or hand off to parent `agent` when needed. |
| **Linear** | Proactively watch issue/comment events from the relay; triage, update status, add comments; optionally invoke or hand off to parent `agent` for deeper work. |
...
1. **Use existing agents**  
   Map `source=github-pr` and `source=linear` to dedicated agent IDs (e.g. `github-agent`, `linear-agent`) in `hooks.mappings`. Those agents must already be defined in `~/.openclaw/agents/`. The relay keeps sending to the same OpenClaw gateway; only the `agentId` in the mapping changes.
...
3. **Parent delegates**  
   The main `agent` profile’s AGENTS.md can list the GitHub and Linear sub-agents. On webhook events, the gateway can still invoke `agent` first; `agent` then delegates to the sub-agent (or the mapping can invoke the sub-agent directly).
...
GitHub/Linear → adnanh/webhook → relay script → sanitize → OpenClaw Gateway
                             ...
                             agentId: github-agent          agentId: linear-agent          agentId: agent
                                    │                             │                             │
                                    ▼                             ▼                             ▼
